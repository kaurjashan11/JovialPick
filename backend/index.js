...
const sendVarifyMail = require('./mail_send');
...
app.post("/upload",upload.single('product'),(req,res)=>{
    res.json({
        success:1,
        image_url: `https://jovial-pick-api.vercel.app/images/${req.file.filename}`
    })
})
...
app.post('/signup',async (req,res)=>{
    ...
    if(check){
        return res.status(400).json({success:false,errors:"User already exist with this email id"})
    }
    let cart = {};
    for (let i = 0; i < 300; i++) {
        cart[i] = 0;
...
app.post('/send-email', async (req, res) => {
    ...
    try {
        const isEmailSent = await sendVarifyMail(email);
        if (isEmailSent) {
